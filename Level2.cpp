...
void Level2::DestroyWall(int Y, int X)
{
	tileDest[Y][X] = 0;
...
void Level2::SetEnemyPos()
{
	...
	Puropen_Enemy2->setPosition(250, 340);
...
int Level2::getGridPosX(int PosX)
{
	int TileX = PosX / _tileWidth;
	return TileX;
}

int Level2::getGridPosY(int PosY)
{
	int TileY = PosY / _tileHeight;
	return TileY;
}

int Level2::getIDfromLayer(int layer, int PosX, int PosY)
{
	int TileX = PosX / _tileWidth;
	int TileY = PosY / _tileHeight;
	if (layer == 0) {
