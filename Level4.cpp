...
void Level4::DestroyWall(int Y, int X)
{
	tileDest[Y][X] = 0;
...
void Level4::SetEnemyPos()
{
	...
	Denkyun_Enemy->setPosition(64, 250);

	Denkyun* Denkyun_Enemy2 = new Denkyun();
	Denkyun_Enemy2->setPosition(384, 143);

	Mechabomb* Mechabomb_Enemys = new Mechabomb();
	Mechabomb_Enemys->setPosition(352, 340);

	EnemiesInLevel.push_back(Denkyun_Enemy);
	EnemiesInLevel.push_back(Denkyun_Enemy2);
	EnemiesInLevel.push_back(Mechabomb_Enemys);
}

int Level4::getTileHeight()
{
	return _tileHeight;
}

int Level4::getTileWidth()
{
	return _tileWidth;
}

int Level4::getHeight()
{
	return _sheetWidth;
}
...
int Level4::getGridPosX(int PosX)
{
	int TileX = PosX / _tileWidth;
	return TileX;
}

int Level4::getGridPosY(int PosY)
{
	int TileY = PosY / _tileHeight;
	return TileY;
}

int Level4::getIDfromLayer(int layer, int PosX, int PosY)
{
	int TileX = PosX / _tileWidth;
	int TileY = PosY / _tileHeight;
	if (layer == 0) {
