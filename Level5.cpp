...
void Level5::DestroyWall(int Y, int X)
{
	tileDest[Y][X] = 0;
...
void Level5::SetEnemyPos()
{
	...
	Denkyun* Denkyun_Enemy = new Denkyun();
	Denkyun_Enemy->setPosition(384, 143);

	NutsStar* NutsStar_Enemys = new NutsStar();
	NutsStar_Enemys->setPosition(192, 15);


	Mechabomb* Mechabomb_Enemys = new Mechabomb();
	Mechabomb_Enemys->setPosition(352, 340);

	/*
	Pakupa* Pakupa_Enemys = new Pakupa();
	Pakupa_Enemys->setPosition(192, 15);
	*/

	EnemiesInLevel.push_back(Puropen_Enemy);
	EnemiesInLevel.push_back(Denkyun_Enemy);
	EnemiesInLevel.push_back(NutsStar_Enemys);
	EnemiesInLevel.push_back(Mechabomb_Enemys);
	//EnemiesInLevel.push_back(Pakupa_Enemys);
}

int Level5::getTileHeight()
{
	return _tileHeight;
}

int Level5::getTileWidth()
{
	return _tileWidth;
}

int Level5::getHeight()
{
	return _sheetWidth;
}
...
int Level5::getGridPosX(int PosX)
{
	int TileX = PosX / _tileWidth;
	return TileX;
}

int Level5::getGridPosY(int PosY)
{
	int TileY = PosY / _tileHeight;
	return TileY;
}

int Level5::getIDfromLayer(int layer, int PosX, int PosY)
{
	int TileX = PosX / _tileWidth;
	int TileY = PosY / _tileHeight;
	if (layer == 0) {
